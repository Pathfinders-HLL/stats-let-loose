...
async def fetch_100killgames_leaderboard(only_pathfinders: bool) -> List[Dict[str, Any]]:
...
def register_100killgames_subcommand(leaderboard_group: app_commands.Group, channel_check=None) -> None:
    ...
    @leaderboard_group.command(
        name="100killgames", 
        description="Get top players with the most 100+ kill games"
    )
    ...
    async def leaderboard_100killgames(
        ...
        if not results:
            await interaction.followup.send(
                "❌ No players found with 100+ kill games.",
                ephemeral=True
            )
            log_command_completion("leaderboard 100killgames", command_start_time, success=False, interaction=interaction, kwargs=log_kwargs)
            return
        ...
        log_command_completion("leaderboard 100killgames", command_start_time, success=True, interaction=interaction, kwargs=log_kwargs)
