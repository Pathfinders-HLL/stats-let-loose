...
-- Creates table for storing player kill statistics by weapon type
-- Based on weapon_schemas.csv weapon types
-- Table name: player_kill_stats

-- Main table for player kill statistics by weapon type
-- Composite primary key: (player_id, match_id)
CREATE TABLE IF NOT EXISTS pathfinder_stats.player_kill_stats (
...
CREATE INDEX IF NOT EXISTS idx_player_kill_stats_match_id ON pathfinder_stats.player_kill_stats(match_id);
CREATE INDEX IF NOT EXISTS idx_player_kill_stats_player_id ON pathfinder_stats.player_kill_stats(player_id);
CREATE INDEX IF NOT EXISTS idx_player_kill_stats_player_name ON pathfinder_stats.player_kill_stats(player_name);

-- Add comments for documentation
COMMENT ON TABLE pathfinder_stats.player_kill_stats IS 'Stores player kill statistics by weapon type for each match. Composite primary key: (player_id, match_id)';
COMMENT ON COLUMN pathfinder_stats.player_kill_stats.player_id IS 'Player Steam ID or unique identifier (part of composite primary key)';
COMMENT ON COLUMN pathfinder_stats.player_kill_stats.match_id IS 'Match/map identifier (part of composite primary key)';
COMMENT ON COLUMN pathfinder_stats.player_kill_stats.player_name IS 'Player display name';
COMMENT ON COLUMN pathfinder_stats.player_kill_stats.team IS 'Team side: either "axis" or "allies"';
COMMENT ON COLUMN pathfinder_stats.player_kill_stats.sniper_rifle IS 'Sniper rifle kills (note: column name reflects typo in source CSV)';
